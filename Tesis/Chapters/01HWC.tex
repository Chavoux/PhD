...
\documentclass[twoside,american,british,english]{elsarticle}
...
\title{Human-Predator Conflict: A new approach\foreignlanguage{english}{
needed}}
\selectlanguage{english}%

\tnotetext{}

\author[rvt]{E. du C.~Luyt\fnref{fn1}\corref{cor1}}

\ead{chavoux@gmail.com}

\author[rvt]{A.J.~Leslie\fnref{fn1}}

\author[rvt]{C.~Hui\fnref{fn1}}

\author[focal]{L.M.~Marker\fnref{fn2}}

\ead[url]{http://www.cheetah.org}

\fntext[fn1]{Stellenbosch University.}

\fntext[fn2]{Cheetah Conservation Fund.}

\cortext[cor1]{Corresponding author}

\address[rvt]{Faculty of AgriSciences, Private Bag X1, 7602 Matieland, South Africa}

\address[focal]{P.O. Box 1755, Otjiwarongo, Namibia}
\selectlanguage{british}%
\begin{abstract}
...
\selectlanguage{english}%
\begin{keyword}
Human-wildife conflict \sep HWC \sep farmer-predator conflict \sep
mitigating methods \sep Namibia \sep sustainability
\end{keyword}
...
\selectlanguage{british}%
Human-wildlife conflict is a growing global problem \citep{Messmer_2000,Treves_n_Karanth_2003b,Nyhus_et_al_2005}.
...
From an ecological perspective it is known that the greatest threat
...
as vulnerable in the IUCN Red List \citep{Durant_et_al_2015}. In
Namibia, the country with the largest surviving population of cheetahs
in the world \citep{Marker_1998,Marker_et_al_2003b,Durant_et_al_2015},
...
In Southern Africa, most of the vulnerable \citep{Stein_et_al_2016}
...
A telephone survey by \citet{Van_Niekerk_2010} showed that livestock
...
Predators fulfil an important role in functioning ecosystems, and
...
Many different methods have been proposed and implemented in the past
to mitigate human-wildlife conflict (HWC). In general these methods
...
Alternative classifications for farmer-predator conflict mitigating
...
\end{enumerate}
\citet{Treves_n_Karanth_2003} classified historic methods to reduce
...
\end{enumerate}
\citet{Linnell_et_al_1996} suggested that zoning should be used to
determine which methods to apply, with 1) a conservation zone (preservation/
...
Given that non-lethal methods are not always \textquotedbl{}non-lethal\textquotedbl{}
...
\end{enumerate}
Using this classification, we can now examine the different methods
...
In parts of Namibia, because of grazing and drinking water limitations,
...
Despite the fact that various manuals with different possible solutions
...
\citep{Durant_et_al_2015}. Farmers are either not using the available
...
A number of different surveys have shown that greater perceived depredation
...
A number of researchers have made the point that a lot of the conflict
...
\citet{Madden_n_McQuinn_2014} showed that material, visible manifestations
...
Process factors relate to decision-making design, equity and authority,
...
\citet{Rust_et_al_2016} make the point that farmer-predator conflict
...
The final and perhaps most obvious part of resolving deep-rooted identity
conflict, involves the actual substance of practical and cost-effective
methods to mitigate farmer-predator conflict. Here the suitability
of a certain method to conditions on a certain farm has often not
been considered sufficiently. A number of researchers have mentioned
that predation losses does not seem to be uniformly distributed, even
within the same district \citep{Conradie_n_Piesse_2013}, and some
farmers might have extremely high depredation losses, while the majority
have relatively few losses \citep{Van_Niekerk_2010}. This appears
to be an international feature of farmer-predator conflict, with similar
reports for example, from Slovakia \citep{Rigg_et_al_2011} and the
USA \citep{Shelton_2004}. But different levels of depredation also
imply that different management decisions and different anti-predation
methods are required.

To summarize some of the main reasons why past human-carnivore conflict
...
\section{How to improve on past failures to resolve human-wildlife conflict?}

\citet{Shivik_2006} opined that future methods \textquotedbl{}need
...
So what should we do differently from what has already been tried?
...
Using a DSS avoids a number of the possible reasons for the past failure
of mitigating methods to reduce HWC. Because it is not a person \textquotedbl{}telling
the farmer what to do on his own farm\textquotedbl{}, at least part
of the common underlying distrust of conservation scientists \citep{Nattrass_n_Conradie_2013}
or past bad experiences with specific conservationists, is avoided
(Axel Rothauge 2014, personal communication). If the DSS is written
in order to be transparent with regards to the algorithms and data
it uses, not only does it make it easier to trust it, but if written
as open source software, it can be updated and improved as new knowledge
and research becomes available. Ultimately it would use feedback from
farmers themselves who are using various methods, to re-evaluate or
update the basic data used (e.g. as costs change or if more limitations
of a specific method is found). A feedback loop is thus built into
the DSS allowing it to adapt to changing circumstances (e.g. changes
in costs of different methods). The algorithm used will include both
the short-term agricultural economic cost-effectiveness of the various
methods and the long-term ecological sustainability in order to choose
the most appropriate method for the specific situation on a farm as
entered by the farmer. This is in effect using an ecologically holistic
view of the farming system, looking at the whole ecosystem and not
considering livestock depredation as an isolated problem \citep{Bingham_1997}.
...
Many different approaches to human-wildlife conflict have been used
...
\section{Details of different HWC mitigating methods}
\selectlanguage{british}%

\subsection{Preventative methods}

Active hunting of predators has been one of the first methods used
...
\end{itemize}
Of these different aims, the selective removing of individual predators
...
last accessed 15 February 2017) which also include the sound of a
radio station in addition to sirens and lights. The costs (in 2008)
were between N\$ 1550.00 and N\$ 1950.00 per unit. A variation on
this method has been used recently around Cape Town to keep baboons
away from residential areas and prevent conflict with people. A simple
timed propane cannon has been used in Namaqualand, Northern Cape,
South Africa to scare baboons away from a kraal with young goat kids
(personal observation). In the USA fladry, in which flags hang from
ropes stretched a short distance above the ground and costing about
US\$ 781.00 per km \citep{Shivik_2006}, is frequently used as a mobile
repellent against wolves \citep{Paige_2015}. Combined with an electrical
line (turbofladry), it costs about US\$ 1328 per km \citep{Shivik_2006}.}
\begin{itemize}
...
last accessed 15 February 2017). They vary greatly in price and effectiveness,
but they all contribute to the ability of predators to distinguish
between natural prey and livestock \citep{Linnell_et_al_1999}. The
e-shepherd is a system where the collar gives an ultrasound alarm
and shine LED lights as soon as the sheep starts running (being chased
by a predators) or leave a certain area. Another high-tech type collar
is the GSM based \textquotedbl{}Veldwagter\textquotedbl{} system \citep{Lotter_2006}. }
\begin{itemize}
...
last accessed 15 February 2017). After initially releasing some caracals
on the land to they put caracal scat around kraals and lambing camps
to keep jackals away from the sheep. }
\begin{itemize}
